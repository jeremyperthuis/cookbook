...
class ListView(Frame):
    def __init__(self, screen, model):
        ...
        self._model = model
        logging.info("id _model : {}".format(id(self._model.contacts)))
        # Create the form for displaying the list of contacts.
        self._list_view = ListBox(
            Widget.FILL_FRAME,
            options=[(x["name"], i) for i,x in enumerate(self._model.contacts)],
            name="contacts",
            add_scroll_bar=True,
...
logging.info("id contacts : {}".format(id(contacts.contacts)))
last_scene = None
while True:
    try:
